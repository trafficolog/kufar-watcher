...
| `0.2` | Postgres в Docker под Dockerode | 0 | ✅ done | 🟢 aligned | 8/8 done: compose Postgres, Dockerode supervision, recoverable bootstrap, existing-container validation, a single POSTGRES_* source-of-truth, live Dockerode CI and safe incompatible-container recovery UX are implemented and verified. |
...
| `1.2` | HTTP-клиент и лимитер | 1 | ✅ done | 🟢 aligned | 5/5 done: global limiter, resilient HTTP client, raw-response journal, stable application-level User-Agent and terminal HTTP response journaling are implemented and verified; 1.2.5 RED #1432 / GREEN #1434. |
...
| `2.7` | Ремедиация повторного аудита фазы 2 | 2 | ✅ done | 🟢 aligned | 12/12 done: remediation фазы 2 закрыта; Run startup recovery теперь подтверждает orphan ownership через canonical per-monitor advisory lease. |
| `3.1` | Телеграм-бот и транспорт | 3 | ✅ done | 🟢 aligned | 3.1.1 bot bootstrap, 3.1.2 reconnect и 3.1.3 durable outbound queue завершены. |
| `3.2` | Форматтер и кнопки | 3 | 🔄 in_progress | 🟡 drifted | Формат уведомления о находке. |
...
| `5.0` | Минимальный пользовательский срез | 5 | 🔄 in_progress | 🟡 drifted | Минимальный UI, замыкающий срез MVP-1: создание монитора и настройка Telegram. |
| `5.1` | Оболочка приложения и трей | 5 | ⬜ todo | 🟡 drifted | Фоновое приложение, которое не нужно держать открытым. |
| `5.2` | Мониторы: список и редактор | 5 | ⬜ todo | 🟡 drifted | Управление правилами из окна приложения; audit gate 5.2.4 требует live scheduler reconciliation после edit mutation. |
...
| `5.8` | Доска наблюдения и первый запуск | 5 | 🔄 in_progress | 🟡 drifted | Доска наблюдения и состояние первого запуска — два состояния одного маршрута. |
